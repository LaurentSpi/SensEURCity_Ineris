...
sensData$PM2.5[which(sensData$outlier_flag=="outlier")]<-NA
sensData=sensData[complete.cases(sensData$PM2.5),]
sensData2=subset(sensData,select=-c(Typology,Clust,Season,Group,outlier_flag,DayType,Periods))
...
for (ntimes in 0:nbr_hours) {
  ...
  if (length(nsensData[, 1]) > 0) {
    ...
    if(numberOfCalibrationGroups>0){
      ...
      # Concatenate
      if (ntimes == 1) {
        calibrationFactorsAlltime3 = calibrationFactors
        calibratedSensorsAlltime3 = calibratedSensors
      } else{
        calibrationFactorsAlltime3 = rbind(calibrationFactorsAlltime3, calibrationFactors)
        calibratedSensorsAlltime3 = rbind(calibratedSensorsAlltime3, calibratedSensors)
      }
...
calibratedSensorsAlltime_LOF <- calibratedSensorsAlltime3

write.csv(
  calibratedSensorsAlltime_LOF,
  file_calibrationFactorsAlltime_csv3,
  row.names = F
)
write.csv(
  calibratedSensorsAlltime_LOF,
  file_calibratedSensorsAlltime_csv3,
  row.names = F
)

save(calibratedSensorsAlltime_LOF, file = file_calibratedSensorsAlltime_csv3)
