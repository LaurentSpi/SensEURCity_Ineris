...
shapefile <- st_read("C:/Users/diallo/OneDrive - INERIS/Documents/Ineris1/Stage_SensEURCity_final/INPUTS1/gadm41_BEL_2.shp")
...
sensData <- LCS_df_all_data_clean_groups_outliers
...
uBss_NoCOLOC_calibrationFactorsAlltime <- data.frame()
uBss_NoCOLOC_calibratedSensorsAlltime <- data.frame()

start <- as.POSIXct("2020-06-18 01:00", format = "%Y-%m-%d %H:%M")
target <- as.POSIXct("2020-12-22 05:00", format = "%Y-%m-%d %H:%M")

# Calcul de la différence en heures
diff_hours <- as.numeric(difftime(target_date, start_date, units = "hours"))
iteration_target <- diff_hours
cat("La date", target_date, "correspond à l'itération ntimes =", iteration_target, "\n")


# Loop over hours
for (ntimes in 4493:4493) {
  ...
  if (length(nsensData[, 1]) > 0) {
    ...
    for (iRef in 1:nrow(referenceData)) {
      ...
      if (length(valid_indices) > 0) {
        ...
        # Exclure les capteurs colocalisés
        ...
        if (length(NoCloseSensors) >= minCountSensor) {
          # tableau large pour calculer `uBss`
    ...
    if(numberOfCalibrationGroups>0){
      ...
      if (nBootstrapSamples > 1) {
        ...
        fileOutName = paste0(outdir,
                             "uBss_PMS_outliers_v2_calibrationFactors_nBootStrap_",
                             nBootstrapSamples)
        fileOutName2 = paste0(outdir2,
                              "uBss_PMS_outliers_v2_calibrationFactors_nBootStrap_",
                              nBootstrapSamples)
        fileOutNameSensor = paste0(outdir,
                                   "uBss_PMS_outliers_v2_calibratedSensors_nBootStrap_",
                                   nBootstrapSamples)
        
      } else {
        fileOutName = paste0(outdir, "uBss_PMS_calibrationFactors")
        fileOutName2 = paste0(outdir2, "uBss_PMS_calibrationFactors")
        fileOutNameSensor = paste0(outdir, "uBss_PMS_calibratedSensors")
      ...
      # Concatenate
      if (ntimes == 1) {
        uBss_NoCOLOC_calibrationFactorsAlltime = calibrationFactors
        uBss_NoCOLOC_calibratedSensorsAlltime = calibratedSensors
      } else{
        uBss_NoCOLOC_calibrationFactorsAlltime = rbind(uBss_NoCOLOC_calibrationFactorsAlltime, calibrationFactors)
        uBss_NoCOLOC_calibratedSensorsAlltime = rbind(uBss_NoCOLOC_calibratedSensorsAlltime, calibratedSensors)
      }
    ...
    # library(ggplot2)
    #
    # provinceNL <- readOGR("C:/Users/gressent/Desktop/Encadrement/STAGES/Stage_SensEURCity_final/SENSEURCITY_CAL/shapeIn/nederlandProvinciesLines.shp")
...
# provinceBEL <- st_read("C:/Users/diallo/OneDrive - INERIS/Documents/Ineris1/Stage_SensEURCity_final/INPUTS1/gadm41_BEL_4.shp")
...
uBss_PMS_calibrationFactorAlltime <- uBss_NoCOLOC_calibrationFactorsAlltime
uBss_PMS_calibratedSensorsAlltime <- uBss_NoCOLOC_calibratedSensorsAlltime

write.csv(
  uBss_PMS_calibrationFactorAlltime,
  paste0(fileOutName, "uBss_PMS_alltime_nmax1000_distmaxRepmax_outliers_NoCOLOC.csv"),
  row.names = F
)
write.csv(
  uBss_PMS_calibratedSensorsAlltime,
  paste0(
    fileOutNameSensor,
    "uBss_PMS_alltime_nmax1000_distmaxRepmax_outliers.csv"
  ),
  row.names = F
)


save(uBss_PMS_calibratedSensorsAlltime,file=paste0(outdir,"uBss_PMS_calibratedSensorsAlltime.Rda"))
