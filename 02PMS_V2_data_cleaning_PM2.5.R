...
# Set directory
setwd("C:/Users/diallo/OneDrive - INERIS/Documents/Ineris1/Stage_SensEURCity_final/")
...
# Load .Rda
#load(paste0(indir,"LCS_PMS_df_all.csv")) # A unique file of sensor data for the whole period
#load(paste0(indir,"ref_df_all.csv")) # A unique file of reference data for the whole period

ref_df_all <- fread("C:/Users/diallo/OneDrive - INERIS/Documents/Ineris1/Stage_SensEURCity_final/INPUTS1/ref_df_all.csv")
save(ref_df_all, file = paste0(indir, "ref_df_all.Rda"))

LCS_PMS_df_all1 <- fread(paste0(indir, "LCS_PMS_df_all.csv"))
save(LCS_PMS_df_all1, file = paste0(indir, "LCS_PMS_df_all.Rda"))
LCS_PMS_df_all <- LCS_PMS_df_all1
...
LCS_PMS_df_all <- as.data.frame(LCS_PMS_df_all)
...
# cette conversion d'unité degré->metre est faite pour la partie calcul de distance...
library(sf)
...
convert_coords <- function(df, lon, lat) {
  ...
  # Création d'un objet sf à partir des données
  df_sf <- st_as_sf(df, coords = c(lon, lat), crs = 4326)
  
  #  # Transformation en coordonnées UTM (zone 31N pour Antwerp, Belgique)
  df_sf_meters <- st_transform(df_sf, crs = 32631)
  
  # Extraction des coordonnées transformées et ajout au dataframe original
...
LCS_PMS_df_all <- LCS_PMS_df_all[!is.na(LCS_PMS_df_all$X) & !is.na(LCS_PMS_df_all$Y), ]

# # Appliquer la fonction de conversion après nettoyage
LCS_PMS_df_all <- convert_coords(LCS_PMS_df_all, "X", "Y")
LCS_PMS_df_all$X <- LCS_PMS_df_all$X.1
LCS_PMS_df_all$Y <- LCS_PMS_df_all$Y.1

# Sauvegarde des dataframes modifiés

write.csv(ref_df_all, paste0(indir, "ref_df_all.csv"), row.names = FALSE)
write.csv(LCS_PMS_df_all, paste0(indir, "LCS_PMS_df_all.csv"), row.names = FALSE)
...
write.csv(ref_df_all, paste0(indir, "ref_df_all.csv"), row.names = FALSE)
save(ref_df_all, file = paste0(indir, "ref_df_all.Rda"))
save(dataout,file=paste0(outdir,"LCS_PMS_df_all_clean.Rda"))
