...
# Load .Rda
load(file_calibratedSensorsAlltime_Rda3) # A modifier en fonction du fichier test
...
head(ref_df_all)
head(calibratedSensorsAlltime_LOF)
...
subset(calibratedSensorsAlltime_LOF, PM2.5 == 0)
colocated_calibratedSensorsAlltime <- calibratedSensorsAlltime_LOF
...
colocated_calibratedSensorsAlltime_LOF <- merge(colocated_calibratedSensorsAlltime, LocID, by = "ID", all.x = TRUE)

save(colocated_calibratedSensorsAlltime_LOF, file=file_colocated_calibratedSensorsAlltime_Rda)
...
plots_generator <- function(station_data, station_id, sensor_ids, path_corr, path_timeseries) {
  ...
  dev.copy(png, filename = file.path(path_timeseries, paste0("Time series_LOF ", station_id_short, " ", paste(sensor_ids, collapse = "-"), ".png")),
  ...
  for (sensor_id in sensor_ids) {
    ...
    dev.copy(png, filename = file.path(path_corr, paste0("Correlation_LOF ", station_id_short, "-", sensor_id, ".png")),
...
for (station_id in names(stations_sensors)) {
  ...
  plots_generator(station_data, station_data_id, sensor_ids, path_correlation_plots4, path_timeseries_plots4) # Modifier en fonction du path test
}
