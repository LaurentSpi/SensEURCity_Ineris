...
PMS_ref_df_all <- as.data.frame(ref_df_all)
LCS_PMS_df_all_clean <- as.data.frame(LCS_df_all_data_clean_groups_outliers)

LCS_PMS_df_all_clean$PM2.5[which(LCS_PMS_df_all_clean$outliers=="outlier")]<-NA
LCS_PMS_df_all_clean=LCS_PMS_df_all_clean[complete.cases(LCS_PMS_df_all_clean$PM2.5),]

#aperçu des deux dataframes 
head(PMS_ref_df_all)
head(LCS_PMS_df_all_clean)

##############""


library(dplyr)
PMS_ref_df_all <- ref_df_all %>% slice(-5825)

# Identifier les stations de référence uniques
stations <- unique(PMS_ref_df_all$ID) 
print(stations)


# Filtrer les données pour les capteurs colocalisés
colocated_LCS_PMS_df_all_clean <- LCS_PMS_df_all_clean %>% 
  filter(grepl("^ANT_REF", Location.ID))

# Voir les premières lignes du nouveau dataframe
head(colocated_LCS_PMS_df_all_clean)
unique(colocated_LCS_PMS_df_all_clean$Location.ID)

# Sauvegarder le nouveau dataframe si nécessaire
save(colocated_LCS_PMS_df_all_clean, file = paste0(outdir, "df_correlations_timeplots_RefSensData_ALL/colocated_LCS_PMS_df_all_clean.Rda"))
...
PMS_ref_df_all <- PMS_ref_df_all %>% rename(date = datetime)
...
station_dfs <- lapply(names(stations_sensors), function(station_id) {
  create_station_df(station_id, stations_sensors[[station_id]], PMS_ref_df_all, colocated_LCS_PMS_df_all_clean)
})
...
output_dir_RefSensData <- "C:/Users/diallo/OneDrive - INERIS/Documents/Ineris1/Stage_SensEURCity_final/OUTPUTS1/df_correlations_timeplots_RefSensData_ALL"
...
save(REF_R801_data, file = paste0(output_dir_RefSensData, "/PMS_REF_R801_data.Rda"))
save(REF_R802_data, file = paste0(output_dir_RefSensData, "/PMS_REF_R802_data.Rda"))
save(REF_R804_data, file = paste0(output_dir_RefSensData, "/PMS_REF_R804_data.Rda"))
save(REF_R805_data, file = paste0(output_dir_RefSensData, "/PMS_REF_R805_data.Rda"))
save(REF_R811_data, file = paste0(output_dir_RefSensData, "/PMS_REF_R811_data.Rda"))
save(REF_R817_data, file = paste0(output_dir_RefSensData, "/PMS_REF_R817_data.Rda"))
save(REF_M802_data, file = paste0(output_dir_RefSensData, "/PMS_REF_M802_data.Rda"))
save(REF_R803_data, file = paste0(output_dir_RefSensData, "/PMS_REF_R803_data.Rda"))
save(REF_AL01_data, file = paste0(output_dir_RefSensData, "/PMS_REF_AL01_data.Rda"))
...
path_corr <- "C:/Users/diallo/OneDrive - INERIS/Documents/Ineris1/Stage_SensEURCity_final/OUTPUTS1/df_correlations_timeplots_RefSensData_ALL/correlation_plots"

path_timeseries <- "C:/Users/diallo/OneDrive - INERIS/Documents/Ineris1/Stage_SensEURCity_final/OUTPUTS1/df_correlations_timeplots_RefSensData_ALL/timeSeries_plots"
